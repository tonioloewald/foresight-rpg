...
**Why the deep-skill version works is the One Rule's arithmetic, and it generalizes far beyond melee.** A large skill advantage lets you back right off and *wear* the penalty yourself, because `SC = Score × EF` is **multiplicative**: a big Score sits at or past certainty, so an EF step costs it **quality**, while a small Score is on the steep part of the curve, where the same step costs **success outright**. Melee 25 off-band is still SC 100; Melee 10 off-band falls from 60 to 40. **Adverse conditions therefore favour the expert** — darkness, rain, broken ground, awkward distance — which is both true to life and almost never modelled, because most systems use *additive* modifiers where −2 is −2 for everybody. It is a free consequence of the sacred rule, and worth stating in the rules text so GMs use it deliberately.
