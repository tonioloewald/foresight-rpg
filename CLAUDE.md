...
- **The ePub cover is real and embedded** (`static/cover.jpg`, 1414×2000, ~581KB — set via `epub.cover` in `site.config.ts`, and it lands in both volumes). It is the web-optimized export of the Affinity master in `legacy/` (gitignored); when Tonio updates the master, re-export and re-optimize to roughly the same size — `sips -s format jpeg -s formatOptions 62` hits it. No `@resvg/resvg-js` needed; that note applied to the old generated-from-title placeholder.
