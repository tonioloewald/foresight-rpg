...
- **Single source of truth = JSON** in `static/data/` (`skills`, `background-factors`, `fields`, `magic-fundamentals`, `magic-applications`, and the equipment collections `weapons`, `armor`, `vehicles`, `spacecraft`, `tools`). Consumed by both the rules pages and the character sheet. Build copies it to `docs/data/`. The build-time markers are `entity-view:` (one collection → table + cards), `matrix:` (cross-tab), `magic-apps:` (one fundamental's apps), `all-applications` and `all-equipment` (flat searchable finders). Any entity may carry an optional `image` (root-relative path) → rendered on its card.
...
- **Character editor** = `src/character-sheet.ts`, a **light-DOM** `<foresight-character-sheet>` built on **tosijs `Component`** (+ tosijs-ui `tosi-field`/`tosi-select`/`tosi-segmented`/`TosiDialog`/`postNotification`), registered via `bundle.ts` and embedded in `src/rules/character-builder.md`. **Light DOM is deliberate and load-bearing: tosijs data bindings do NOT work inside a shadow root** (a shadow component is bound like an `<input>` — value in, change out), so isolation comes from `static lightStyleSpec` (a stylesheet scoped to the tag, `:host` semantics) instead of a shadow boundary. It was Shadow DOM before precisely because its CSS used bare `body`/`h1`/`table` selectors; those are now scoped. Rules live in **`src/character-rules.ts`** (pure functions — the point economy, conferred attrs/skills/fields, formula scores, max-level clamping; run `bun run check` to exercise them), and persistence behind **`src/character-store.ts`** (`CharacterStore`; localStorage today, Firestore later — key `foresight_characters_v2`). Derived views are rendered explicitly in `render()`: **`queueRender()` dedupes against the component's own attribute/value tracking, and this state lives outside it, so a queued render gets dropped** — call `render()` directly after a change. Also: `input({ list: … })` throws (`HTMLInputElement.list` is read-only) — set it via `apply(el)` as an attribute. Web-only by design (excluded from the ePub; the book links to the printable sheet).
...
- **The ePub cover is real and embedded** (`static/cover.jpg`, 1414×2000, ~581KB — set via `epub.cover` in `site.config.ts`, and it lands in both volumes). It is the web-optimized export of the Affinity master in `legacy/` (gitignored — the masters are backed up to iCloud, so don't propose committing the 34MB `.afphoto` files); when Tonio updates the master, re-export and re-optimize to roughly the same size — `sips -s format jpeg -s formatOptions 62` hits it. No `@resvg/resvg-js` needed; that note applied to the old generated-from-title placeholder.
...
- **Wounds & exhaustion:** wound level **0→6** (6=dead, 4=incapacitated, 5=dying-without-intervention), −(level) to everything; damage = **Yield × DC × ½, rounded to nearest**, and raw ≥ ¼ forces a stun check. Fatigue/magic/divine favour are one **Exhaustion** track (0→5, 4=incapacitated, 5=collapsed/may-die; full to own domain, ½ to others, fatigue hits all; tracks stack). Pain Resistance roll (EN/WP) → shrug / Shaken / Stunned. Recovery: convalescence rolls (wounds) / rest deepest-first (exhaustion); divine favour is earned back by deeds (Piety), not rested. See the **Damage** and **Exhaustion** one-pagers (they consolidated the old damage/wounds-and-recovery/power-pools pages).
...
- **Story comes from friction — but friction must cost *decisions*, not *duration* or *bookkeeping*.** Remove resistance (Skyrim: you can't even struggle uphill) and the world is scenery. But the same friction fails or works **by medium**: fatigue-and-weather slowdown sank a 90s CRPG, yet it's the load-bearing heart of *The Left Hand of Darkness*. **The variable is compression** — can you get the ordeal's meaning without undergoing its duration? Tabletop sits near the novel *only if the rules allow it*. **Requirement: the unit of resolution must scale to the narrative unit** — a week's march is a check or three, not forty rolls. That's why exhaustion is a level track checked when you push, turn length is elastic, and pursuit runs at a longer tempo. **Third condition: friction needs a *countermove that pays*** — an uphill penalty in Skyrim would only help if route-reading were rewarded; otherwise it's a tax. **Full test: compressible · answerable · rewarded.** Fail the first → tedium; fail the others → a tax. **And friction *is* the reward** — story is what it pays in — so **every path needs its own friction**: an alternative that can't go wrong is an exit, not an option. Corollary for the unwritten chapters (Interpersonal, Travel, Religion, NPCs): they need combat's **consequence**, not its page count. *Players go where the system has teeth.*
