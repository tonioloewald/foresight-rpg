...
- [ ] **Remove/merge `quality-ratings.md` + `open-ended-resolution.md`** now that One Rule subsumes their cores. The **partial-blocking table** (attack-QR × defence-QR) is combat material — it wasn't carried into Damage; decide whether it lands on Damage, Initiative, or a dedicated melee note (2004 has the parry/dodge QR mechanics: parry BEF = attack QR, QR5+ blocks/reduces). The tennis example can fold into One Rule or go.
