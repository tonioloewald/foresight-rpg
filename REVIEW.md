...
- [x] ~~**Best-effort text cleanup**~~ **Done** — all 10 chapters cleaned (~975 OCR fixes) via per-chapter agents under strict fidelity rules (fix OCR only, never reword/renumber); verified with a numbers+markers script + word-diff review. Reflow now uses the original's indentation (no more run-on paragraphs). Source PDF (gs-compressed 95→37MB) is a downloadable asset with per-page `#page=N` deep-links.
