...
- [ ] **Label the quirk *families* explicitly** — Appearance, Empathy, Coordination, Age — each with its combine rule. Default **best-applicable-only within a family** (same as tools/armor); exception where two members fire for *different* reasons (Appearance *Gorgeous* + Appearance *Scarred* can both apply — they're not two ends of one dial). The at-table test: *do these fire on this roll for the same reason?*
...
- [ ] **Chronic conditions as quirks** (Tonio, 2026-08) — write the four-line template (attach / trigger / effect / management) into the quirks rules page, plus a starter set. It is deliberately a *template*, not a catalogue: the space is endless, so shipping the ability to write one in a minute beats shipping a list. Worked examples to include:
...
- [ ] **Severity = write it twice.** Mild and severe asthma are two quirks, not a quirk with a dial. Applies to the aging quirks too.
- [ ] **Acquired conditions** — a wound that heals badly becomes a chronic quirk via the same template. Cross-reference from `damage.md` recovery; this is the honest replacement for a scar table.
