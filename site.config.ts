...
export default defineSiteConfig({
  ...
  // Two ePub volumes from one build: the default (streamlined edition) and a
  // separate "foresight-1986" volume for the restoration. Docs join the 1986
  // volume via `"book": "foresight-1986"` in their metadata (set on the section
  // landing, inherited by its child pages); everything else lands in the default.
  epub: {
    author: 'Tonio Loewald',
    cover: 'static/cover.jpg',
    volumeTitles: { 'foresight-1986': 'ForeSight 1986 — Original Edition (community restoration)' },
  },
  ...
  book: {
    ...
    // The `1986-*` pages are NOT excluded — they're routed to their own ePub
    // volume (see epub.volumeTitles + the `book` field on src/docs/foresight-1986.md).
